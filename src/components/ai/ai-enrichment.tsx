...
import { JinaReaderService, JinaReaderResult } from '../../services/jina-reader-service';
import { ContentFetchErrorDialog } from '../website/content-fetch-error-dialog';
...
export function AIEnrichment({ website, onUpdateWebsite, aiConfig }: AIEnrichmentProps) {
  ...
  const [websiteContent, setWebsiteContent] = useState<JinaReaderResult | null>(null);
  ...
  // 获取网站内容
  const fetchWebsiteContent = async (): Promise<JinaReaderResult> => {
    setIsFetchingContent(true);
    setCurrentTask('获取网站内容');
    setProgress(20);

    try {
      const result = await JinaReaderService.getWebsiteContent(website.url);
      setWebsiteContent(result);
      
      if (!result.success) {
        throw new Error(result.error || '获取网站内容失败');
      }
      
      return result;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : '获取网站内容失败';
  ...
  // 处理内容获取错误
  const handleContentFetchError = (error: string) => {
    setFetchError(error);
  ...
  // 不获取内容直接进行AI增强
  const handleContinueWithoutContent = async () => {
    setShowErrorDialog(false);
    await performAIEnrichment('all');
  };

  // 执行AI增强
  const performAIEnrichment = async (type: 'all' | string = 'all') => {
    setCurrentTask('AI智能分析');
    setProgress(60);
    ...
    try {
      ...
      const enrichmentContext = {
        website,
        websiteContent: websiteContent?.success ? websiteContent : undefined,
        enrichmentType: type as 'description' | 'tags' | 'features' | 'related' | 'reviews' | 'analysis' | 'all'
      };

      const result = await enrichmentService.enrichWebsite(enrichmentContext);
  ...
  const handleStartEnrichment = async (type: 'all' | string = 'all') => {
    ...
    try {
      ...
      try {
        ...
        // 内容获取失败，显示错误弹窗
        handleContentFetchError(error instanceof Error ? error.message : '获取网站内容失败');
        return; // 等待用户选择
      }

      // 第二步：执行AI增强
      await performAIEnrichment(type);
      
    } catch (error) {
      console.error('增强过程失败:', error);
      setError(error instanceof Error ? error.message : '增强过程失败');
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <ContentFetchErrorDialog
        ...
        onContinueWithoutContent={handleContinueWithoutContent}
        error={fetchError}
