...
import { Bot, Sparkles, Search, TrendingUp, Settings, Plus, CheckCircle, AlertCircle, History, Tag } from 'lucide-react';
import { toast } from 'sonner';
import { Website, Tag as TagType } from '../../types/website';
import { AIService, AIConfig, AIRecommendationRequest, AIRecommendationResponse, StreamChunk, AIWebsiteRecommendation } from '../../services/ai-service';
import { JinaSearchService, JinaSearchResult } from '../../services/jina-search-service';
import { AIConfigDialog } from './ai-config-dialog';
import { AIChatHistory, ChatMessage } from './ai-chat-history';
import { BatchAddDialog } from '../data-management/batch-add-dialog';
import { StreamingPreview } from '../streaming-preview';
import { AIWebsiteCard } from './ai-website-card';
...
export function AIRecommendation({ onAddWebsite }: AIRecommendationProps) {
  ...
  // Jina Search 相关状态
  const [jinaSearchResults, setJinaSearchResults] = useState<JinaSearchResult | null>(null);
  const [isSearching, setIsSearching] = useState(false);
  const [useJinaSearch, setUseJinaSearch] = useState(true);
  ...
  useEffect(() => {
    ...
    if (savedConfig) {
      try {
        ...
        // 如果解析失败，使用默认配置
        setAiConfig(AIService.getDefaultConfig());
      }
    } else {
      // 如果没有保存的配置，确保使用默认配置
      setAiConfig(AIService.getDefaultConfig());
  ...
  const loadExistingData = async () => {
    try {
      const appData = dataManager.getLocalData();
      
      // 获取所有标签
      const allTags: TagType[] = [];
      appData.websites.forEach((website: Website) => {
        website.tags.forEach((tagName: string) => {
          if (!allTags.find(tag => tag.name === tagName)) {
            allTags.push({
  ...
  const setDefaultCategories = () => {
    const defaultCategories = [
      { id: 'cat_1', name: '开发工具', color: '#3b82f6' },
      { id: 'cat_2', name: '设计工具', color: '#ef4444' },
      { id: 'cat_3', name: 'AI工具', color: '#10b981' },
      { id: 'cat_4', name: '效率工具', color: '#8b5cf6' },
      { id: 'cat_5', name: '学习资源', color: '#f59e0b' },
      { id: 'cat_6', name: '商业工具', color: '#06b6d4' },
      { id: 'cat_7', name: '团队协作', color: '#84cc16' },
      { id: 'cat_8', name: '娱乐休闲', color: '#ec4899' }
    ];
  ...
  const handleAIRecommend = async () => {
    ...
    try {
      // 第一步：如果启用了Jina Search，先进行搜索
      let searchContext = '';
      if (useJinaSearch && aiConfig.jinaApiKey) {
        setIsSearching(true);
        try {
          const jinaService = new JinaSearchService({ apiKey: aiConfig.jinaApiKey });
          const searchResult = await jinaService.search(query.trim());
          
          if (searchResult.success && searchResult.results) {
            setJinaSearchResults(searchResult);
            ...
            console.log('Jina Search结果:', searchResult);
          } else {
            console.warn('Jina Search失败:', searchResult.error);
          }
        } catch (searchError) {
          console.warn('Jina Search请求失败:', searchError);
        } finally {
  ...
  const handleAddWebsite = (aiSite: AIWebsiteRecommendation) => {
    try {
      ...
      const aiService = new AIService(aiConfig);
      const website = aiService.convertToWebsite(aiSite, selectedCategory === 'none' ? '' : selectedCategory);
      ...
      if (selectedCategory && selectedCategory !== 'none') {
        console.log('将标签添加到分类:', selectedCategory);
        AIService.addTagsToCategory(aiSite.tags, selectedCategory);
      }
      ...
      // 显示成功提示
      const successMessage = selectedCategory && selectedCategory !== 'none' 
        ? `网站"${website.title}"已添加成功，标签已自动归入"${selectedCategory}"分类。`
        : `网站"${website.title}"已添加成功。`;
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select value={selectedCategory} onValueChange={setSelectedCategory}>
                        ...
                        <SelectContent>
                          <SelectItem value="none">不指定分类</SelectItem>
                  ...
                  <Button 
                    ...
                    disabled={!query.trim() || isAnalyzing || isSearching}
