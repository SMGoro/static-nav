...
import { initializeDefaultData, checkDataIntegrity } from './utils/data-initializer';
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    if (!integrityCheck.isComplete) {
      console.log('检测到数据不完整，正在初始化默认数据...');
      const initResult = initializeDefaultData();
      if (initResult.success) {
        console.log('默认数据初始化成功:', initResult.dataCount);
        // 重新加载数据
        setAppData(dataManager.getLocalData());
      } else {
        console.error('默认数据初始化失败:', initResult.message);
      }
    }
    ...
    if (shareData) {
      ...
      if (!localData.websites || localData.websites.length === 0) {
        // 如果没有数据，加载默认的mock数据
  ...
  const renderCurrentView = () => {
    ...
    if (path === '/data') {
      return (
        <DataManager onDataUpdate={setAppData} />
      );
    }
  ...
  const navigationItems = [
    ...
    },
    {
      key: '/ai',
      icon: Bot,
      label: t('navigation.aiRecommendation'),
    ...
    },
    {
      key: '/test',
      icon: TestTube,
      label: t('navigation.shareTest'),
      action: () => handleNavigation('/test')
    },
    {
      key: '/add',
      icon: Plus,
      label: t('navigation.addWebsite'),
          ...
          <div className="w-full px-4 sm:px-6 py-3 sm:py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center min-w-0">
                <h2 className="text-sm sm:text-xl mr-3 sm:mr-6 flex-shrink-0">{t('messages.welcome')}</h2>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="border-t pt-2 mt-2">
                          <div className="text-xs text-muted-foreground text-center px-3 py-1">
                            {t('navigation.websiteCount', { count: websites.length })}
                          </div>
                        </div>
                      </div>
                    </DropdownMenuContent>
