...
export function DataRestoreManager({ currentData, onDataRestore }: DataRestoreManagerProps) {
  ...
  const getInitialDataStats = () => {
    ...
    return {
      ...
      tagRelations: initialData.tagRelations.length
    };
  };
  ...
  const getCurrentDataStats = () => {
    return {
      ...
      tagRelations: currentData.tagRelations.length
    };
  };
...
• ${currentStats.tagRelations} 个标签关系

将恢复为：
• ${initialStats.websites} 个网站
• ${initialStats.tags} 个标签
• ${initialStats.tagRelations} 个标签关系
...
• 所有标签关系 (${currentData.tagRelations.length} 个)
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center p-4 border rounded-lg">
              <div className="text-2xl font-bold text-purple-600">{currentStats.tagRelations}</div>
              <div className="text-sm text-muted-foreground">标签关系</div>
            </div>
          </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center p-4 border rounded-lg bg-muted/50">
              <div className="text-2xl font-bold text-purple-600">{initialStats.tagRelations}</div>
              <div className="text-sm text-muted-foreground">标签关系</div>
              <div className="text-xs text-muted-foreground mt-1">
                预设的标签关联
              </div>
